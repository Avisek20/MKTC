...
for n_bags_size in [3000, 4000, 6000, 7000, 8000]:
    ...
    for i in range(len(datasets)):
        ...
        if not os.path.exists(op_folder+datasets[i]+'_n'+str(n_bags_size)):
            os.makedirs(op_folder+datasets[i]+'_n'+str(n_bags_size))
        ...
        for i_set in range(n_set):
            print(i_set+1,' / 10')
            tmp = np.load(
                'data_bags_npz'+'_n'+str(n_bags_size)+'/'+datasets[i]+'/'+ datasets[i] +'_set'+ str(i_set) +'.npz'
            )
            ...
            np.save(op_folder+datasets[i]+'_n'+str(n_bags_size)+'/'+datasets[i] +'_MImem_set'+ str(i_set) +'.npy', multi_instance_mem)
            np.save(op_folder+datasets[i]+'_n'+str(n_bags_size)+'/'+datasets[i] +'_w_set'+ str(i_set) +'.npy', w)
            np.save(op_folder+datasets[i]+'_n'+str(n_bags_size)+'/'+datasets[i] +'_centers_set'+ str(i_set) +'.npy', centers)
            np.save(op_folder+datasets[i]+'_n'+str(n_bags_size)+'/'+datasets[i] +'_costs_set'+ str(i_set) +'.npy', costs)
            np.save(op_folder+datasets[i]+'_n'+str(n_bags_size)+'/'+datasets[i] +'_datamem_set'+ str(i_set) +'.npy', data_mem)
            ...
            with open(op_folder+datasets[i]+'_n'+str(n_bags_size)+'/'+datasets[i] +'_res.txt', 'a') as fa:
                fa.write(
                    str(i_set) + ' '

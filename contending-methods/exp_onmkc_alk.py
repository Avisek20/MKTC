...
from onmkc_alk import onmkc_alk
...
if __name__ == '__main__':
    ...
    for adataset in range(len(datasets)):
        ...
        start = time.time()
        H, J, beta, v_iter = onmkc_alk(K, n_clusters=n_clusters, max_iter=100, n_init=10)
        exec_time = time.time() - start

        np.save(output_folder+datasets[adataset]+'_H.npy', H)
        np.save(output_folder+datasets[adataset]+'_J.npy', J)
        np.save(output_folder+datasets[adataset]+'_beta.npy', beta)

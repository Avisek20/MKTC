...
if __name__ == '__main__':
    ...
    for adataset in range(len(datasets)):
        ...
        start = time.time()
        H, G, gamma, v_iter = onkcmk(K, n_clusters=n_clusters, max_iter=100, n_init=10)
        exec_time = time.time() - start

        np.save(output_folder+datasets[adataset]+'_H.npy', H)
        np.save(output_folder+datasets[adataset]+'_G.npy', G)
        np.save(output_folder+datasets[adataset]+'_gamma.npy', gamma)
